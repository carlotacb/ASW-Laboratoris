...
function CurrencyConverterPlus($currencies){
    ...
    $curren = $currencies->to_Currencies;
    $amo = $currencies->amount;
    
    $result = array();
    
    foreach($curren as $cur) {
        $ConversionResult = new stdClass();
        $ConversionResult->currency = $cur;
        $ConversionResult->amount = CurrencyConverter($from, $cur, $amo);
        array_push($result, $ConversionResult);
    }
