...
try{
    ...
    $inCur = "CNY";
	$outCur = array("EUR", "CAD", "TRY");
	$inAmount = 1000;
	
	$ConversionRequest = new stdClass();
	$ConversionRequest->from_Currency = $inCur;
	$ConversionRequest->to_Currencies = $outCur;
	$ConversionRequest->amount = $inAmount;
	
	$outAmount = $sClient->CurrencyConverterPlus($ConversionRequest);
	echo "\n   ", $inAmount, " ",$inCur,"\n";
	foreach ($outAmount as $out) {
        echo "      ==> ", $out->amount, " ", $out->currency, "\n";
	}
