...
public class WoTServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		PrintWriter TwIDsortida = response.getWriter ();
		String author = request.getParameter("author");
		String tw = request.getParameter("tweet_text");
		...
		if (TwID != null) {
			if (cookies != null) {
				for (Cookie c: cookies) {
					if (c.getValue().equals(convertirSHA256(TwID))) {
			...
			try {
				idtw = Database.insertTweet(author, tw);
				response.addCookie(new Cookie("Twid" + convertirSHA256(idtw.toString()), convertirSHA256(idtw.toString())));
		...
		if (!header.equals("text/plain")) {
			...
			TwIDsortida.print(String.valueOf(idtw));
		}
	}
	...
	private void printPLAINresult (Vector<Tweet> tweets, HttpServletRequest req, HttpServletResponse res) throws IOException
	{
		...
		for (Tweet tweet: tweets) {
			...
			out.println("Tweet #" + tweet.getTwid() + ": " + tweet.getAuthor() + ": "+ tweet.getText() + " [" + messDate + " " + timeFormatter.format(tweet.getDate())+ "]");
		}
	}
